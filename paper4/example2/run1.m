...
% proj(M)*(C*[x1;x2])
figure(1);clf;
subplot(2,1,1);cla;hold on;grid on;legend("show");
xlabel("time(sec)");title("Measured Output");

xhat=pinv(proj(M)*C)*(proj(M)*phi);
plot(t,phi(1,:),'m','LineWidth',2,'DisplayName','y_1');
plot(t,phi(2,:),'c','LineWidth',2,'DisplayName','y_2');
plot(t,phi(3,:),'g','LineWidth',2,'DisplayName','y_3');
plot(t,phi(4,:),'k','LineWidth',2,'DisplayName','y_4');

subplot(2,1,2);cla;hold on;grid on;legend("show");
xlabel("time(sec)");title("States");
plot(t,x1,'k','LineWidth',3,'DisplayName','x_1');
plot(t,x2,'k','LineWidth',3,'DisplayName','x_2');
