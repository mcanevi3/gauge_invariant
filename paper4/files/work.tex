...
\begin{corollary}[Component Decoupling and Recovery]
For the measurement equation $y = Cx + D_w w$ and under assumptions $\mathrm{rank}(C) < p$ and $\mathrm{rank}(D_w) < p$, the following decoupling and recovery properties hold:
\begin{enumerate}
    ...
    \item (\textit{Recoverability}) The disturbance $w$ can be uniquely recovered via $\hat{w} = (\Pi^C D_w)^\dagger \Pi^C y$ if and only if $\mathcal{R}(D_w) \not\subseteq \mathcal{R}(C)$, ensuring $\Pi^C D_w \neq 0$.
\end{enumerate}
\end{corollary}
...
\begin{proof}
    ...
    Similarly, for (\textit{Disturbance invariance}), $\Pi^{D_w} (Cx + D_w w)=\Pi^{D_w} w$.
    
    The condition $\mathcal{R}(D_w) \not\subseteq \mathcal{R}(C)$ ensures that $D_w$ is not in the null space of $\Pi^C$ and by the \textit{Existence} property, $\Pi^C D_w$ is remains, which proves (\textit{Recoverability}).
\end{proof}


\begin{corollary}[Component Decoupling and Recovery Generalization]
