...
\begin{gather}
    \mathrm{rank}(C)<p
\end{gather}
arises in sparse sensor applications\cite{shoukry2015event}, topologies used in Multi Agen Systems(MAS)\cite{olfati2007consensus} and distributed networks \cite{yang2022sensor}, and is also called Strictly Output Redundant(SOR) system \cite{yang2025output}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{gather}
    \Pi^C\triangleq I-C(C^TC)^{-1}C^T \label{eq:projection}
\end{gather}
which satisfies,
\begin{gather}
    \Pi^C C= (I-C(C^TC)^{-1}C^T)C=0
\end{gather}
Let $C^\perp=\mathcal{N}(C^T)$,
\begin{gather}
    \Pi^C C^\perp= (I-C(C^TC)^{-1}C^T)C^\perp=C^\perp
\end{gather}
and for the projection the following holds,
\begin{gather}
    (\Pi^C)^2=\Pi^C,\,\Pi^C\in\mathbb{R}^{p\times p}
\end{gather}

Using the projection operator on the measured output yields,
\begin{gather}
    \Pi^C y=\Pi^C(C x+D_w w)=\Pi^C D_w w
\end{gather}
...
\begin{gather}
    \hat{\omega}=(\Pi^C D_w)^\dagger(\Pi^C y)
\end{gather}
where $(.)^\dagger$ is the Moore-Penrose pseudo-inverse. This recovery is possible if and only if,
\begin{gather}
