...
Let $X\in \mathbb{R}^{nxn}$ be a matrix with $\mathrm{rank}(X)=n<s$. There exists a unique orthogonal projection matrix $\Pi^X\in\mathbb{R}^{sxs}$ that maps the space $\mathbb{R}^s$ onto the left null space $\matcal{N}(X^T)$, defined by:
\begin{equation*}
