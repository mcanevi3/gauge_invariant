...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}
A Luenberger observer is designed solving the LMI given in (\ref{eqn:lmi1}) and is obtained as,
\begin{equation}
    L=\begin{bmatrix}
...
On the other hand, using the projection filtering the projection matrix given by (\ref{eq:projection}) is calculated for $C$ as,
\begin{equation}
\Pi^C=\begin{bmatrix}
    0.5&   -0.5&         0\\
    -0.5&    0.5&         0\\
...
The estimation process of the Luenberger observer takes $0.04$ seconds due to the fact that the observer is a dynamic observer, whereas the projection filtering directly matches the original system states at $t=0$ seconds as illustrated in Figure~\ref{fig:ex1_states1}. It is possible to decrease the estimation duration of the observer by speeding up the observer eigenvalues, but in return the observer gain and therefore noise amplification would increase. 
...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}
In order to be able to recover both the states and the disturbance in such a system, the number of sensors for measurement needs to increase due to the condition given in (\ref{eqn:rank_cond_proj}).
...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}
and the projection defined in (\ref{eq:projection}) to recover the state information $x(t)$ can be calculated using,
\begin{equation}
